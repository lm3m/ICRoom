...
class test_create_user(unittest.TestCase):
    ...
    def test_count(self):
        ...
        for topic_count in range(0, count):
            ...
            time.sleep(.1)
            self.assertIsNotNone(models.topics_model.TopicsModel.create_topic("{}".format(topic_count), "fake topic {}".format(topic_count)))

        topics = models.topics_model.TopicsModel.list_topics()
